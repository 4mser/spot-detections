    ...
    scene.traverse(child => {
      if (child.isMesh) {
        child.material = new THREE.MeshStandardMaterial({
          ...
          transparent: true,
          opacity: 0.8, // Ajusta la opacidad según la transparencia deseada
        });
      }
    });
  }, [scene]);
...
function PyramidModel({ positionAngle }) {
  ...
  const { opacity } = useSpring({
    ...
    from: { opacity: 0 },
    to: { opacity: 0.01 },
    config: { duration: 1000 }, // Ajusta la duración según lo deseado
    to: async (next) => {
  ...
  return (
    <animated.mesh geometry={pyramidGeometry} material-opacity={opacity} b>
      <meshBasicMaterial color="cyan" transparent side={THREE.DoubleSide} />
    </animated.mesh>
  );
}

function MiniScene({ positionAngle }) {
  const scale = 0.1;  // Escala de la miniatura
  const yOffset = 10;  // Desplazamiento vertical para la miniatura
  ...
  return (
    <>
      <SpotModel positionAngle={positionAngle} {...transformProps} />
      <PyramidModel positionAngle={positionAngle} {...transformProps} />
      <WindowModel {...transformProps} />
    </>
  );
}
...
const SpotRobot = ({onAngleSelect}) => {
  ...
  return (
    <>
      <div className='flex z-10 w-full    md:w-1/2 bottom-0 py-10 absolute justify-center '>
        {angleOptions.map((angle) => (
          <motion.button
            ...
            className={`${displayAngle === angle ? 'bg-black' : 'bg-black/30'} shadow-md`}
          >
            {`${angle}°`}
          </motion.button>
        ))}
      ...
      <div className='h-[100dvh] w-full md:w-1/2 hover:cursor-grab'>
        <Canvas shadows camera={{ position: [0, 5, -10], fov: 60 }} >
          ...
          <Suspense fallback={null}>
            <MiniScene positionAngle={positionAngle} />
